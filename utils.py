...
def train_one_epoch_K(model, optimizer, data_loader, device, epoch, lr_scheduler):
    ...
    for step, data in enumerate(data_loader):
        ...
        images = images.float()
        pred = model(images.to(device))   #
        pred_classes = torch.max(pred, dim=1)[1]  # 类别index
        # accu_num += torch.eq(pred_classes, labels.to(device)).sum()
        labels_true = torch.argmax(labels, dim=1)  # 类别index
...
def evaluate_K(model, data_loader, device, epoch):
    ...
    for step, data in enumerate(data_loader):
        ...
        labels_true = torch.argmax(labels, dim=1)  # 类别index
        accu_num += torch.eq(pred_classes.to(device), labels_true.to(device)).sum()
        loss = loss_function(pred, labels.to(device).float())  # 形状(batch_size, num_classes)
        accu_loss += loss
...
def evaluate(model, data_loader, device, epoch):
    ...
    for step, data in enumerate(data_loader):
        ...
        # f1 += f1_score(y_true=labels_cpu, y_pred=pred_classes_cpu, average='macro')
        # n = n+1
        # print('f1:',f1)
        # 计算f1 ---------------
